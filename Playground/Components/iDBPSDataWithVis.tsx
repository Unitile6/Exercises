...
export function createKeypoint(name: string, x: number, y: number, connections: string[]): { name: string, coordinates: { x: number, y: number }, connections: string[] } {
    return {
        name: name,
        coordinates: { x: x, y: y },
...
export function KeypointsWithSVG() {
        ...
        const interval = setInterval(() => {
            setKeypoints(prevKeypoints => {
                return prevKeypoints.map(keypoint => {
                    // Random movement within a range of -20 to +20 pixels on both x and y axes
    ...
    return (
        <svg id="svg-container" viewBox="0 0 300 300" width="500" height="500">
