...
export function lowerCoordinates(pathData: string | undefined) {
  ...
  // Modify the coordinates to set the initial coordinate to 0,0
  const updatedPathData = commands.map(command => {
    const values = command.trim().split(/\s+/);
    const cmd = values.shift();
    let updatedValues = [];
    for (let i = 0; i < values.length; i += 2) {
      const x = parseFloat(values[i]) - minX;
      const y = parseFloat(values[i + 1]) - minY;
      updatedValues.push(x, y);
    }
    return `${cmd} ${updatedValues.join(' ')}`;
  }).join(' ');

  return updatedPathData;
}
/* export function panZoomRotate(){
console.clear();
